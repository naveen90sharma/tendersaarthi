...
async function migrateTable(tableName) {
    ...
    // Sample a few rows to get all possible columns
    ...
    // Determine types from first row that has the column
    const colDefs = columns.map(col => {
        const rowWithCol = data.find(r => r[col] !== null && r[col] !== undefined) || data[0];
        const val = rowWithCol[col];
        ...
        else if (col.endsWith('_at') || col.endsWith('_date') || col.endsWith('_ts') || col === 'bid_submission_end' || col === 'bid_opening_date') type = 'TIMESTAMPTZ';
...
async function run() {
    // List of ALL essential tables to ensure full coverage
    const tables = [
